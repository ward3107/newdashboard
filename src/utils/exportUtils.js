...
export const exportToExcel = async (students) => {
  ...
  try {
    ...
    const excelData = students.map(student => ({
      ...
      'מספר חוזקות': student.strengthsCount,
      'מספר אתגרים': student.challengesCount,
...
/**
 * Export students summary to PDF
 * @param {Array} students - Array of student objects
 * @returns {Promise} Download promise
 */
export const exportToPDF = async (students) => {
  ...
  try {
    ...
    const avgStrengths = (students.reduce((sum, s) => sum + s.strengthsCount, 0) / students.length).toFixed(1);
    const avgChallenges = (students.reduce((sum, s) => sum + s.challengesCount, 0) / students.length).toFixed(1);
    ...
    students.forEach((student, index) => {
      ...
      const rowData = [
        student.studentCode,
        student.classId,
        student.strengthsCount.toString(),
        student.challengesCount.toString()
      ];
...
/**
 * Export single student detailed report to PDF
...
export const exportStudentDetailToPDF = async (studentData) => {
  ...
  try {
    ...
    // Learning Style
    if (studentData.student_summary?.learning_style) {
      doc.setFontSize(14);
      doc.text('סגנון למידה', pageWidth - margin, yPosition, { align: 'right' });
      yPosition += 8;

      doc.setFontSize(10);
      const learningStyles = studentData.student_summary.learning_style.split('\n');
      learningStyles.forEach(style => {
        if (style.trim()) {
          doc.text(style.replace(/^• /, ''), pageWidth - margin, yPosition, { align: 'right' });
          yPosition += 6;
        }
      });
      yPosition += 8;
    ...
    // Strengths
    if (studentData.student_summary?.strengths) {
      doc.setFontSize(14);
      doc.text('חוזקות', pageWidth - margin, yPosition, { align: 'right' });
      yPosition += 8;

      doc.setFontSize(10);
      studentData.student_summary.strengths.forEach(strength => {
        doc.text(strength, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 6;
      });
      yPosition += 8;
    }
    ...
    // Challenges
    if (studentData.student_summary?.challenges) {
      doc.setFontSize(14);
      doc.text('אתגרים', pageWidth - margin, yPosition, { align: 'right' });
      yPosition += 8;

      doc.setFontSize(10);
      studentData.student_summary.challenges.forEach(challenge => {
        doc.text(challenge, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 6;
      });
      yPosition += 8;
    }
    ...
    // Immediate Actions
    if (studentData.immediate_actions) {
      if (yPosition > 200) {
        doc.addPage();
        yPosition = 20;
      }
      ...
      doc.setFontSize(14);
      doc.text('פעולות מיידיות', pageWidth - margin, yPosition, { align: 'right' });
      yPosition += 8;

      doc.setFontSize(10);
      studentData.immediate_actions.forEach((action, index) => {
        doc.text(`${index + 1}. ${action.what}`, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 6;
        doc.text(`   כיצד: ${action.how}`, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 6;
        doc.text(`   מתי: ${action.when}`, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 6;
        doc.text(`   זמן: ${action.time}`, pageWidth - margin, yPosition, { align: 'right' });
        yPosition += 8;
...
export const generatePrintHTML = (studentData) => {
  return `
    ...
    <body>
      ...
      ${studentData.immediate_actions ? `
        <div class="section">
          <h2>פעולות מיידיות</h2>
          ${studentData.immediate_actions.map((action, index) => `
            <div style="margin-bottom: 15px; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
              <h4>${index + 1}. ${action.what}</h4>
              <p><strong>כיצד:</strong> ${action.how}</p>
              <p><strong>מתי:</strong> ${action.when}</p>
              <p><strong>זמן:</strong> ${action.time}</p>
            </div>
          `).join('')}
        </div>
      ` : ''}
